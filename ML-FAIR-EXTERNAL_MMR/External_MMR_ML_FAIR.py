"""External MMR ML-FAIR - Analysis of external mean-motion resonances."""
import argparse
...
def compute_sigma_elements_external(planet, asteroid, primary):
    """Compute the external resonant angle sigma in the invariable plane.

    Sigma = (Omega + omega + M)_asteroid - (Omega + omega + M)_planet
    """
...
def simular_asteroide(args):
    ...
    for k, t in enumerate(times):
        ...
        sigma_rad = compute_sigma_elements_external(
...
def main():
    """Main entry point for external MMR analysis."""
    ...
    parser = argparse.ArgumentParser(description="Compute external MMR locations.")
    ...
    for (idx, name, a_input, _log, _fname, _nps, _npm, ncs, ncm, ns, nM, flagged, _ffn) in results:
        ...
        if p > 0 and q >= 0:
            pq = p + q
            a_res_au = analysis.resonance_a_outer(a_planet, p, pq)
