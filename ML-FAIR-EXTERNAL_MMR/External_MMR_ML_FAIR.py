...
#   python unified_external_ensemble.py --planet mercury
...
# ============================================================
# Resonant angle in invariable plane
# ============================================================
...
# ============================================================
# Outer resonance location using Kepler's third law
# ============================================================
...
def get_planet_name():
    parser = argparse.ArgumentParser(
        description="Compute external MMR locations with ML-FAIR for a selected planet."
    )
    parser.add_argument(
        "--planet",
        type=str,
...
print(f"Selected planet: {PLANET_CFG['label']}")
...
def simular_asteroide(args):
    ...
    if flagged:
        fair_filename = f"res_forgacs_{SELECTED_PLANET}_{name}_FAIR_FLAGGED.png"
    ...
    plt.tight_layout()
    filename = f"res_forgacs_{SELECTED_PLANET}_{name}{'_FLAGGED' if flagged else ''}.png"
...
def main():
    ...
    for (
        ...
        if np.isfinite(p) and np.isfinite(q) and (p > 0) and (q >= 0):
            pq = int(p + q)
            a_res_au = resonance_a_outer(a_planet, int(p), pq)
    ...
    if summary_rows:
        ...
        summary_file = f"resonant_summary_{SELECTED_PLANET}.csv"
