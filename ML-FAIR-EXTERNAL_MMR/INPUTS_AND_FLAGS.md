...
The current version of `External_MMR_ML_FAIR.py` expects local files with fixed names in the working directory.
...
For the external code, the script then interprets these values as:
...
The theoretical external resonance location is then computed as:

```text
a_res = a_planet * ((p + q) / p)^(2/3)
```

and the candidate is retained in the summary only when:

```text
Delta_a = |a_res - a_input| < 0.4 AU
```
...
## Useful implementation notes from the current script

A few details are easy to miss when setting up runs:

- the selected planet can be passed with `--planet` or chosen interactively;
- the code uses `TkAgg`, so local graphical support may be required when flagged figures are displayed;
- figures are written with names like `res_forgacs_<planet>_<asteroid>_FLAGGED.png` and `res_forgacs_<planet>_<asteroid>_FAIR_FLAGGED.png`;
- the summary CSV is written as `resonant_summary_<planet>.csv`.
